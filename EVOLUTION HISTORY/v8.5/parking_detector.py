...
class ParkingDetector:
    def __init__(self, config: Optional[ParkingDetectorConfig] = None):
    ...
    def verify_parallel(self, zone: ParkingZone) -> bool:
        diff_mm = abs(
            zone.left_distance_m - zone.right_distance_m
        ) * 1000.0

        return diff_mm <= self.config.parallel_threshold_mm

    def _extract_markers(self, frame: Any) -> List[Marker2D]:
        # Replace with actual vision detection.
        return []

    def _compute_from_four(
        self, markers: List[Marker2D]
    ) -> ParkingZone:
    ...
    def _estimate_from_partial(
        ...
        return ParkingZone(
            ...
            width_m=0.5,
            length_m=0.3,
            left_distance_m=0.15,
            right_distance_m=0.15,
